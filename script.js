document.addEventListener('DOMContentLoaded', () => {
    const D = {
        ...
        n: JSON.parse(localStorage.n || '[]'),
        cc: +localStorage.cc || 0,
        cu: JSON.parse(localStorage.cu || 'null')
    };
    function S() {
        ['u', 'p', 'g', 'm', 'n'].forEach(k => localStorage[k] = JSON.stringify(D[k]));
        localStorage.cc = D.cc;
        localStorage.cu = JSON.stringify(D.cu);
    }
    ...
    document.getElementById('login-btn').onclick = () => {
        ...
        if (!u) {
            let r = 'guest';
            if (p === '959506' && D.cc < 2) { r = 'creator'; D.cc++; }
            u = { id: Date.now().toString(), username: n, password: p, role: r, premium: false, avatar: '', banUntil: null, color: r === 'creator' ? 'gold' : H(n, p), followers: [], following: [] };
            D.u.push(u);
        } else {
            if (u.password !== p) return alert('Неверный пароль!');
        ...
        D.cu = { id: u.id, username: u.username, role: u.role };
        S(); SH();
    };
    ...
    function ST(tab, arg) {
        ...
        if (tab === 'feed') RF();
        else if (tab === 'groups') RGr();
        else if (tab === 'messages') RC();
        else if (tab === 'profile') RP();
        else if (tab === 'notifications') RN();
        else if (tab === 'admin') RA();
    }

    document.getElementById('publish-btn').onclick = () => {
        const u = U(); if (!u || (u.banUntil && u.banUntil > Date.now())) return alert('Ты забанен');
        const tx = document.getElementById('post-text').value.trim(),
              im = document.getElementById('post-image').files[0],
              vi = document.getElementById('post-video').files[0];
        ...
        Promise.all(pr).then(m => {
            const po = { id: Date.now().toString(), authorId: u.id, text: tx, media: m[0] || null, timestamp: Date.now(), likes: [], comments: [] };
            D.p.unshift(po); S();
            document.getElementById('post-text').value = ''; document.getElementById('post-image').value = ''; document.getElementById('post-video').value = '';
            ST('feed');
        });
    };
    ...
    function RF() {
        ...
        D.p.forEach(po => {
            const au = D.u.find(u => u.id === po.authorId); if (!au) return;
    ...
    function RP() {
        ...
        if (isMe) {
            document.getElementById('change-avatar').onclick = () => { const f = document.getElementById('avatar-upload').files[0]; if (f) { const r = new FileReader(); r.onload = () => { u.avatar = r.result; S(); RP(); }; r.readAsDataURL(f); } };
        } else {
            ...
            document.getElementById('add-grp-btn').onclick = () => {
                const gn = prompt('Название группы:'); if (!gn) return;
                const g = D.g.find(g => g.name === gn && (g.creatorId === me.id || g.admins.includes(me.id)));
                if (!g) return alert('Группа не найдена или нет прав!');
                if (g.members.includes(u.id)) return alert('Уже в группе!');
    ...
    function AN(uid, type, actor) { const txt = type === '❤️' ? `${actor.username}, ${R(actor)}, лайкнул(а) пост.` : `${actor.username}, ${R(actor)}, прокомментировал(а) пост.`; D.n.push({ id: Date.now().toString(), userId: uid, type, text: txt, read: false, timestamp: Date.now() }); S(); UNB(); }
    function RN() { const u = U(), l = document.getElementById('notifications-list'); if (!l) return; l.innerHTML = '<h3>Уведомления</h3>' + (u ? D.n.filter(n => n.userId === u.id).reverse().map(n => { n.read = true; return `<div class="glass" style="padding:8px">${n.type} ${n.text} <small>${F(n.timestamp)}</small></div>`; }).join('') : ''); S(); UNB(); }
    ...
    function VG(gid) {
        const g = D.g.find(g => g.id === gid), u = U(); if (!g) return; const v = document.getElementById('group-view'); v.classList.remove('hidden');
        ...
        if (g.creatorId === u.id) {
            document.getElementById('ren-grp').onclick = () => { const nn = prompt('Новое имя:', g.name); if (nn) { g.name = nn; S(); VG(gid); } };
    ...
    function RA() { const u = U(); if (!u || u.role !== 'creator') return; const l = document.getElementById('admin-users-list'); l.innerHTML = '<h3>Все пользователи</h3>' + D.u.map(u => { const ban = u.banUntil && u.banUntil > Date.now(); return `<div class="glass" style="display:flex;justify-content:space-between;margin:5px 0;padding:10px;"><span>${u.username} – ${R(u)} ${ban ? '⛔' : ''}</span><div><button class="btn-small ban-btn" data-userid="${u.id}">⛔ Бан</button><button class="btn-small prem-btn" data-userid="${u.id}">⭐ Премиум</button></div></div>`; }).join(''); document.querySelectorAll('.ban-btn').forEach(b => b.onclick = () => { const u = D.u.find(x => x.id === b.dataset.userid); const d = prompt('Дни бана (0 - разбан)'); if (d !== null && u) { u.banUntil = d > 0 ? Date.now() + d * 86400000 : null; S(); RA(); } }); document.querySelectorAll('.prem-btn').forEach(b => b.onclick = () => { const u = D.u.find(x => x.id === b.dataset.userid); if (u && u.role !== 'creator') { u.premium = !u.premium; S(); RA(); } }); }
    function RC(openUserId = null) {
        const u = U(); const l = document.getElementById('chats-list');
        l.innerHTML = '<h3>Диалоги</h3>' + D.u.filter(x => x.id !== u.id).map(x => `<div class="user-chat" data-userid="${x.id}" style="cursor:pointer">${x.username} (${R(x)})</div>`).join('');
        document.querySelectorAll('.user-chat').forEach(d => d.onclick = () => OC(d.dataset.userid));
        if (openUserId) OC(openUserId);
    }
    function OC(pid) {
        const u = U(), p = D.u.find(u => u.id === pid); if (!p) return;
        const cw = document.getElementById('chat-window'); cw.classList.remove('hidden');
        const cid = [u.id, p.id].sort().join('_'); let c = D.m.find(c => c.chatId === cid);
        if (!c) { c = { chatId: cid, messages: [] }; D.m.push(c); }
    ...
    // Автовход
    if (D.cu) {
        const u = D.u.find(x => x.id === D.cu.id);
        if (u && (!u.banUntil || u.banUntil <= Date.now())) SH();
        else { D.cu = null; S(); }
    }
});const proc=(f,t)=>new Promise(res=>{const r=new FileReader();r.onload=()=>res({type:t,data:r.result});r.readAsDataURL(f)})
const pr=[];if(im)pr.push(proc(im,'image'));if(vi)pr.push(proc(vi,'video'))
